...
Let's have a look at the cluster file from the example data. It consists of 2 columns containing the geneID and the cluster for all the DE genes.
...
Load POSTAR and ENCODE RBP data. Those are 2 public datasets which are processed in order to have lists of vector. Each vector is named after 1 RBP and contains the geneID of all the targets of that RBP.
...
res_Postar=CLIPReg_V4(RBP_data=RBP_POSTAR,cluster=clusters)
res_Encode=CLIPReg_V4(RBP_data=RBP_ENCODE,cluster=clusters)
save(res_Encode,file="Res_RBP_Encode.RData")
save(res_Postar,file="/Fibroblasts/Res_RBP_Postar.RData")
```
...
res=combine(res1=res_Encode,res2=res_Postar)

```

Load the log2foldchange and identify the RBPs
...
Plot_GO_RBP(rbp_of_interest="QKI",Targets=Targets,clusters=clusters,GO_to_show=3)
```
